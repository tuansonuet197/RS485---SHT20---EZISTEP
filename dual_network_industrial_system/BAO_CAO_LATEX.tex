...
Trong bối cảnh Cách mạng công nghiệp 4.0 phát triển mạnh mẽ, việc tự động hóa và giám sát các hệ thống công nghiệp trở nên vô cùng quan trọng. Mạng truyền thông công nghiệp, đặc biệt là RS-485, đóng vai trò then chốt trong việc kết nối và điều phối các thiết bị trong môi trường sản xuất.

Tuy nhiên, thực tế triển khai cho thấy mỗi thiết bị công nghiệp lại sử dụng một giao thức truyền thông khác nhau. Ví dụ, cảm biến môi trường thường dùng Modbus RTU (chuẩn mở, phổ biến), trong khi các driver động cơ hiện đại như Ezi-STEP lại sử dụng giao thức độc quyền (FASTECH Protocol) để tối ưu hóa điều khiển chuyển động.

Đề tài này được lựa chọn nhằm nghiên cứu và xây dựng một hệ thống giám sát môi trường và điều khiển động cơ trên nền tảng hai mạng RS-485 độc lập, đồng thời tích hợp và làm chủ cả hai giao thức Modbus RTU (chuẩn công nghiệp mở) và FASTECH Protocol (giao thức chuyên dụng cho điều khiển động cơ). Việc kết hợp đa giao thức này không chỉ giúp sinh viên hiểu sâu về các chuẩn truyền thông thực tế, mà còn nâng cao khả năng tích hợp hệ thống, đáp ứng yêu cầu đa dạng của các ứng dụng công nghiệp hiện đại.

Đây là một thách thức kỹ thuật thực sự, đòi hỏi khả năng nghiên cứu tài liệu, lập trình giao tiếp, xử lý dữ liệu và đảm bảo tính ổn định, an toàn cho hệ thống tự động hóa.
...
\begin{itemize}
    ...
    \item Phát triển hệ thống tự động hóa thông minh dựa trên dữ liệu cảm biến.
    \item Áp dụng kiến thức lý thuyết vào thực tế.
\end{itemize}

% --- USB to RS485 CH340 ---
\subsection{USB to RS485 CH340}
	extbf{Chức năng:} Kết nối máy tính với mạng RS-485 qua cổng USB, sử dụng chip CH340 phổ biến, hỗ trợ Windows/Linux.

	extbf{Thông số chính:}
\begin{itemize}
    \item Giao tiếp: USB 2.0 sang RS-485
    \item Chip: CH340
    \item Điện áp hoạt động: 5V (lấy từ USB)
    \item Tốc độ truyền: 300~115200 bps
    \item Đầu nối: Terminal block 2 chân (A/B)
    \item Hỗ trợ: Windows, Linux, MacOS
    \item Link tham khảo: \url{https://lkcg.vn/usb-to-rs485-ch340}
\end{itemize}

	extbf{Ứng dụng trong dự án:} Dùng làm converter USB-RS485 cho cả SHT20 và Ezi-STEP.

% --- Nguồn tổ ong 24V 3A ---
\subsection{Nguồn tổ ong 24V 3A}
	extbf{Chức năng:} Cung cấp nguồn DC 24V ổn định cho driver Ezi-STEP và động cơ BM-42M.

	extbf{Thông số chính:}
\begin{itemize}
    \item Điện áp vào: 220V AC (lưới điện dân dụng)
    \item Điện áp ra: 24V DC
    \item Dòng tối đa: 3A (72W)
    \item Loại: Nguồn tổ ong (switching power supply)
    \item Kích thước: 110 x 78 x 36 mm (tham khảo)
    \item Bảo vệ: Quá dòng, quá áp, ngắn mạch
    \item Link tham khảo: \url{https://shopee.vn/B%E1%BB%99-Chuy%E1%BB%83n-%C4%90i%E1%BB%87n-T%E1%BB%95-Ong-220V-AC-sang-24V-DC-3A.-i.11732493.93080175}
\end{itemize}

	extbf{Ứng dụng trong dự án:} Cấp nguồn cho driver Ezi-STEP và động cơ BM-42M.
...
\begin{itemize}
    ...
    \item Nghiên cứu FASTECH Protocol qua tài liệu hướng dẫn giao tiếp.
...
\begin{itemize}
    ...
    \item Thiết kế kiến trúc phần mềm (module, sơ đồ lớp).
    \item Thiết kế giao diện người dùng (mockup).
    \item Thiết kế quy tắc tự động hóa.
\end{itemize}

	\textbf{3. Triển khai:}
\begin{itemize}
    \item Xây dựng driver cho từng thiết bị.
    \item Phát triển giao diện GUI với PyQt5.
    \item Tích hợp các module vào hệ thống hoàn chỉnh.
    \item Gỡ lỗi và sửa lỗi từng bước.
\end{itemize}

	\textbf{4. Kiểm thử và đánh giá:}
\begin{itemize}
    \item Kiểm thử đơn vị cho từng module.
    \item Kiểm thử tích hợp cho toàn hệ thống.
    \item Kiểm thử độ ổn định (stress test 8 giờ).
    \item Đo lường hiệu năng (thời gian đáp ứng, sử dụng CPU/RAM).
...
\begin{itemize}
    ...
    \item Hệ thống tự động hóa với 4 quy tắc cơ bản.
    \item Giao diện GUI desktop (PyQt5) chạy trên PC Windows/Linux.
    \item Ghi log dữ liệu định dạng CSV.
\end{itemize}

	\textbf{Ngoài phạm vi đề tài:}
\begin{itemize}
    \item Web dashboard và giám sát từ xa.
    \item Ứng dụng di động (Android/iOS).
    \item Cơ sở dữ liệu lưu trữ dài hạn (MySQL, MongoDB).
    \item Ứng dụng machine learning dự đoán.
    \item Bộ điều khiển PID cho vị trí chính xác.
...
\begin{itemize}
    ...
    \item Giới thiệu RS-485: nguyên lý truyền tín hiệu vi sai, so sánh với các chuẩn khác.
    \item Kiến trúc mạng kép độc lập và lý do lựa chọn.
\end{itemize}
...
\begin{itemize}
    ...
    \item 4 ví dụ minh họa chi tiết với phân tích gói tin.
    \item Hệ thống tự động hóa: kiến trúc, quy tắc, tính năng nâng cao, triển khai mã nguồn.
\end{itemize}
...
\section{Tổng quan về giao thức truyền thông FASTECH Protocol}

\subsection{Giới thiệu về FASTECH Protocol}
FASTECH Protocol là giao thức truyền thông độc quyền do hãng Fastech (Hàn Quốc) phát triển, chuyên dùng cho các dòng driver động cơ bước Ezi-STEP. Giao thức này tối ưu cho điều khiển chuyển động công nghiệp, hỗ trợ nhiều loại lệnh (JOG, MOVE, STOP, SERVO ON/OFF, đọc trạng thái, v.v.) với tốc độ truyền cao và khả năng kiểm soát chi tiết.

		extbf{Đặc điểm chính:}
\begin{itemize}
    \item Giao thức độc quyền, tối ưu cho điều khiển động cơ bước
    \item Hỗ trợ nhiều loại lệnh chuyển động và truy vấn trạng thái
    \item Tốc độ truyền cao (mặc định 115200 bps, hỗ trợ đến 460800 bps)
    \item Có kiểm tra lỗi CRC-16 CCITT
    \item Sử dụng byte stuffing để tránh nhầm lẫn với byte đặc biệt
    \item Khung truyền có header (0xAA 0xCC) và tail (0xAA 0xEE)
\end{itemize}

\subsection{Cấu trúc gói tin FASTECH Protocol}

Mỗi gói tin FASTECH có cấu trúc:
\begin{center}
	exttt{[Header][ID][CMD][DATA...][CRC][Tail]}
\end{center}
\begin{itemize}
    \item \textbf{Header:} 2 byte (0xAA 0xCC)
    \item \textbf{ID:} 1 byte (địa chỉ driver)
    \item \textbf{CMD:} 1 byte (mã lệnh: JOG, MOVE, STOP...)
    \item \textbf{DATA:} N byte (tham số lệnh, ví dụ tốc độ, vị trí...)
    \item \textbf{CRC:} 2 byte (CRC-16 CCITT, Little Endian)
    \item \textbf{Tail:} 2 byte (0xAA 0xEE)
\end{itemize}
	extit{Nếu trong phần dữ liệu xuất hiện byte 0xAA, sẽ chèn thêm 0x00 ngay sau đó (byte stuffing) để tránh nhầm lẫn với header/tail.}

		extbf{Ví dụ gói lệnh JOG:}
\begin{verbatim}
AA CC 01 37 00 64 00 32 12 34 AA EE
Header  ID CMD  Speed  Acc  CRC  Tail
\end{verbatim}

\subsection{Ưu điểm và nhược điểm của FASTECH Protocol}
\begin{itemize}
    \item \t\textbf{Ưu điểm:}
    \begin{itemize}
        \item Điều khiển động cơ chi tiết, nhiều chế độ chuyển động
        \item Tốc độ truyền cao, phản hồi nhanh
        \item Có kiểm tra lỗi CRC, byte stuffing tăng độ tin cậy
        \item Hỗ trợ phản hồi trạng thái động cơ, cảnh báo, alarm
    \end{itemize}
    \item \t\textbf{Nhược điểm:}
    \begin{itemize}
        \item Giao thức độc quyền, ít tài liệu công khai
        \item Cần tự xây dựng hàm đóng/gỡ gói, tính CRC, byte stuffing
        \item Không phổ biến như Modbus, khó tích hợp thiết bị khác hãng
    \end{itemize}
\end{itemize}
...
\subsubsection{Cấu hình DIP switch trên driver Ezi-STEP}

Trên thân driver Ezi-STEP Plus-R có một cụm DIP switch dùng để cấu hình các thông số truyền thông như địa chỉ (Slave ID) và tốc độ truyền (Baud rate). Việc gạt đúng các switch này là bắt buộc để driver giao tiếp được với phần mềm.

	extbf{Ý nghĩa các switch (tham khảo tài liệu Ezi-STEP):}
\begin{itemize}
    \item SW1~SW4: Thiết lập địa chỉ Slave ID (nhị phân, 0~15)
    \item SW5~SW8: Thiết lập Baud rate và các chế độ khác
\end{itemize}

	extbf{Bảng cấu hình Baud rate (SW5~SW8):}
\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|c|c|}
\hline
	extbf{SW5} & \textbf{SW6} & \textbf{SW7} & \textbf{SW8} & \textbf{Baud rate} \\
\hline
OFF & OFF & OFF & OFF & 9600 \\
ON  & OFF & OFF & OFF & 19200 \\
OFF & ON  & OFF & OFF & 38400 \\
ON  & ON  & OFF & OFF & 57600 \\
OFF & OFF & ON  & OFF & 115200 \\
ON  & OFF & ON  & OFF & 230400 \\
OFF & ON  & ON  & OFF & 460800 \\
\hline
\end{tabular}
\caption{Thiết lập Baud rate bằng DIP switch trên driver Ezi-STEP}
\end{table}

	extbf{Ví dụ thực tế:}

\begin{itemize}
    \item Để chọn Baud rate 115200: Gạt SW5 = OFF, SW6 = OFF, SW7 = ON, SW8 = OFF
    \item Địa chỉ Slave ID = 1: Gạt SW1 = ON, SW2 = OFF, SW3 = OFF, SW4 = OFF (tương ứng nhị phân 0001)
\end{itemize}

	extit{Lưu ý: Sau khi thay đổi DIP switch, cần tắt/bật lại nguồn driver để cấu hình có hiệu lực.}
...
\vspace{0.3cm}
\small\textit{Hình ảnh bộ chuyển đổi USB to RS-485 (COM1: SHT20, COM2: Ezi-STEP)}
\vspace{3cm}
}}
...
\begin{tikzpicture}[scale=0.85, transform shape,
    ...
    \node[device, fill=yellow!15, below=0.6cm of term2a, minimum height=0.9cm] (ezi) {\small\t\textbf{Ezi-STEP}\\{\tiny Slave 2}};
        % Motor BM-42M dưới Ezi-STEP
        \node[device, fill=red!15, below=0.8cm of ezi, minimum height=0.9cm] (motor) {\small\textbf{BM-42M}\\{\tiny Motor}};
    ...
    \draw[very thick, blue!70] (ezi.south) -- (motor.north);
    \draw[very thick, blue!70] (motor.south) -- (term2b);
...
\begin{tikzpicture}[scale=0.75, transform shape, node distance=1.5cm,
    ...
    % Master
    \node[master] (master) {\t\textbf{Master}\\PC - Python\\COM1: SHT20 | COM2: Ezi-STEP};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Bus 1 - Left side (Modbus RTU)
        \node[term, below=1.3cm of com1] (term1a) {};
        \node[left=0.1cm of term1a, font=\scriptsize] {120$\Omega$};
        \node[device, fill=green!15, below=0.6cm of term1a, minimum height=0.9cm] (sht20) {\small\textbf{SHT20}\\{\tiny Slave 1}};
        \node[term, below=0.6cm of sht20] (term1b) {};
        \node[left=0.1cm of term1b, font=\scriptsize] {120$\Omega$};
        % Bus 1 lines
        \draw[very thick, red!70] (com1.south) -- (term1a);
        \draw[very thick, red!70] (term1a) -- (sht20.north);
        \draw[very thick, red!70] (sht20.south) -- (term1b);
        ...
        % Bus 2 - Right side (FASTECH)
        \node[term, below=1.3cm of com2] (term2a) {};
        \node[right=0.1cm of term2a, font=\scriptsize] {120$\Omega$};
        \node[device, fill=yellow!15, below=0.6cm of term2a, minimum height=0.9cm] (ezi) {\small\textbf{Ezi-STEP}\\{\tiny Slave 2}};
        % Motor BM-42M dưới Ezi-STEP
        \node[device, fill=red!15, below=0.8cm of ezi, minimum height=0.9cm] (motor) {\small\textbf{BM-42M}\\{\tiny Motor}};
        \node[term, below=0.6cm of motor] (term2b) {};
        \node[right=0.1cm of term2b, font=\scriptsize] {120$\Omega$};
        % Bus 2 lines
        \draw[very thick, blue!70] (com2.south) -- (term2a);
        \draw[very thick, blue!70] (term2a) -- (ezi.north);
        \draw[very thick, blue!70] (ezi.south) -- (motor.north);
        ...
        \draw[thick] (master.south) -- (com2.north);
    \end{tikzpicture}
    \caption{Sơ đồ khối kết nối hệ thống: PC, 2 mạng RS-485 độc lập, SHT20 và Ezi-STEP}
\end{figure}
...
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.8, transform shape, node distance=0.7cm,
    ...
    loop/.style={rectangle, draw, thick, minimum width=3.5cm, minimum height=2.2cm, align=center, fill=yellow!10},
    decision/.style={diamond, draw, thick, minimum width=2.2cm, minimum height=1.2cm, align=center, fill=orange!20, aspect=2},
    arrow/.style={thick, ->, >=stealth}]
    
    \node[start] (start) {\scriptsize START};
    \node[process, below=of start] (init) {\scriptsize Init GUI\\Init Drivers};
    \node[process, below=of init] (connect) {\scriptsize Kết nối COM1 (SHT20)\\Kết nối COM2 (Ezi-STEP)};
    \node[process, below=of connect] (thread) {\scriptsize Start Threads\\SHT20 | Ezi-STEP | GUI};
    
    \node[loop, below=of thread] (mainloop) {
        \scriptsize
        		extbf{MAIN LOOP}\\[0.1cm]
        Read SHT20 (1s)\\
        Read Motor Status (0.5s)\\
        Update GUI (Real-time)\\
        Check Auto Rules
    };
    % Thêm bước Chạy motor
    \node[process, below=0.9cm of mainloop] (runmotor) {\scriptsize Chạy Motor (nếu có lệnh)};
    \node[decision, below=1.1cm of runmotor] (exit) {\scriptsize User\\Exit?};
    \node[process, below=0.8cm of exit] (cleanup) {\scriptsize Stop Motor\\Close Ports\\Cleanup};
    \node[start, below=of cleanup, fill=red!20] (end) {\scriptsize END};
    
    \draw[arrow] (start) -- (init);
    \draw[arrow] (init) -- (connect);
    \draw[arrow] (connect) -- (thread);
    \draw[arrow] (thread) -- (mainloop);
    \draw[arrow] (mainloop) -- (runmotor);
    \draw[arrow] (runmotor) -- (exit);
    \draw[arrow] (exit) -- (cleanup) node[midway, right, font=\tiny] {Yes};
    \draw[arrow] (exit.east) -- ++(1.2,0) |- (mainloop.east) node[near start, right, font=\tiny] {No};
...
\clearpage
\appendix
% --- Start appendix with Roman page numbering ---
\cleardoublepage
...
\begin{enumerate}
    \item \t\textbf{Kết nối SHT20 (COM1):}
    \begin{itemize}
        \item USB-Serial Converter → PC (ghi nhận số COM, VD: COM1)
        \item RS-485 A/B → SHT20 A/B
        \item Nguồn 5V → SHT20 VCC/GND
        \item Cấu hình DIP switch: Slave ID = 1, Baud = 9600
    \end{itemize}
    
    \item \t\textbf{Kết nối Ezi-STEP (COM2):}
    \begin{itemize}
        \item USB-Serial Converter → PC (ghi nhận số COM, VD: COM2)
...
\begin{enumerate}
    ...
    \begin{lstlisting}[language=Python]
# config.py
SHT20_COM_PORT = "COM1"  # COM1: SHT20
SHT20_BAUDRATE = 9600

EZISTEP_COM_PORT = "COM2"  # COM2: Ezi-STEP
...
\subsubsection{Tab 1: Giám sát SHT20 (COM1)}
\begin{itemize}
...
\subsubsection{Tab 2: Điều khiển Motor (COM2)}
\begin{itemize}
